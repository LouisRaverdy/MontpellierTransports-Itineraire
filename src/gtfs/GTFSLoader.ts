...
import {pushNested, setNested} from "../ts-utils/ts_utils";
...
export function loadGTFS(stream: Readable): Promise<GTFSData> {
  ...
  const processor = {
    ...
    trip: row => {
      trips.push({ serviceId: row.service_id, tripId: row.trip_id, routeId: row.route_id, directionId: row.direction_id, stopTimes: [], service: {} as Service });
    },
    stop_time: row => {
      const stopTime = {
        stop: row.stop_id,
